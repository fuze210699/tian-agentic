...
export const VITE_ENDPOINT = '/_tian-annotate/api';
...
export function annotationsPath(root: string) {
  return join(ensureDir(root), 'annotations.json');
}

export function readAnnotations(root: string): Annotation[] {
...
export function writeAnnotations(root: string, annotations: Annotation[]) {
...
// ---------------------------------------------------------------------------
// Dispatch (same logic as tian-vue-annotate-server dispatch.ts, simplified
// for Vite context — no sessions.config.json; cwd is always the Vite root)
// ---------------------------------------------------------------------------
...
// Resolve Windows .cmd shims to the real .exe so we can spawn with
// shell:false and pass argv entries untouched (shell:true re-splits on spaces).
...
export interface TianAnnotatePluginOptions {
  /** Override the API prefix. Default: /_tian-annotate/api */
  endpoint?: string;
}

export function tianAnnotate(options: TianAnnotatePluginOptions = {}): Plugin {
  const prefix = options.endpoint ?? VITE_ENDPOINT;
  let root: string;

  return {
    name: 'tian-vue-annotate',
    apply: 'serve',
    ...
    configureServer(server) {
      server.middlewares.use(async (req, res, next) => {
        ...
        if (method === 'GET' && path === '/health') {
          ...
          return sendJson(res, 200, { ok: true, mode: 'vite-middleware', defaultAgent: cfg.agent ?? null });
        }
